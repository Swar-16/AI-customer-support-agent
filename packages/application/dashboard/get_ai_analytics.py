# AI-customer-support-agent\packages\application\dashboard\get_ai_analytics.py
...
from packages.application.dashboard.analytics_models import AIAnalyticsResult
...
class GetAIAnalytics:
    """Return sanitized AI analytics for administrators."""
    ...
    def execute(self, query: GetAIAnalyticsQuery) -> AIAnalyticsResult:
        if not isinstance(query, GetAIAnalyticsQuery):
            raise TypeError("query must be a GetAIAnalyticsQuery.")
        ...
        result = repository.get_ai_analytics(window=query.window)
        if not isinstance(result, AIAnalyticsResult):
            raise TypeError("repository must return an AIAnalyticsResult.")
